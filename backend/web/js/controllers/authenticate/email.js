    ...
    $scope.opendellist = function(aciton){
        ...
        if(rows.length < 1){
            ...
            if(confirm('确定要解除认证此会员吗？解除认证后需要重新填写信息！！')){
                $http.post('member/authenticate/email-do', {id: rowid}).success(function (data) {
                    if(data){
                        alert("解除邮箱认证成功！");
                        $state.reload();
                    }else{
...
app.controller('emailCtrl', ['$scope', '$http','gridSharedService','$state','$filter', function($scope, $http,gridSharedService,$state, $filter) {
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        setTimeout(function () {
            ...
            if (searchText) {
                ...
                $http.get('member/authenticate/email').success(function (largeLoad) {
                    data = largeLoad.filter(function(item) {
                        if(item.email_status==10){
                            item.email_status ="正常";
                        }else{
                            item.email_status ="锁定";
                        }
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
            ...
            {field:'email', displayName:'认证邮箱'},
            {field:'phone', displayName:'电话'},
            {field:'email_status', displayName:'认证状态'},
