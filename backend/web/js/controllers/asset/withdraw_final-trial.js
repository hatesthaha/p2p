...
app.controller('withdrawfinalMenuCtrl',['$scope', 'gridSharedService','$http', '$state', function($scope, gridSharedService, $http, $state){
    //编辑按钮组
    $scope.editGroup = {
        btnNew: true,
        btnEdit: false,
    ...
    $scope.opendellist = function(aciton){
        ...
        if(rows.length < 1){
            ...
            if(confirm('确定要审核此提现吗？')){
                $http.post('asset/withdraw/final-trial', {id: rowid}).success(function (data) {
                    if(data){
                        alert("审核通过！");
                        $state.reload();
                    }else{
...
app.controller('withdrawfinalCtrl', ['$scope', '$http','gridSharedService','$state','$filter', function($scope, $http,gridSharedService,$state, $filter) {
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        setTimeout(function () {
            ...
            if (searchText) {
                ...
                $http.get('asset/withdraw/final-trial').success(function (largeLoad) {
