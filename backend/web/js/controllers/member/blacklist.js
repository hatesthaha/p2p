...
app.controller('ToolBarMoreMenuCtrl',['$scope', 'gridSharedService','$http', '$state', function($scope, gridSharedService, $http, $state){
    ...
    $scope.opendellist = function(aciton){
        ...
        if(rows.length < 1){
            ...
            if(confirm('确定要为此会员解锁吗？')){
                $http.post('member/members/unlock', {id: rowid}).success(function (data) {
                    if(data){
                        alert("解锁成功！");
                        $state.reload();
                    }else{
...
app.controller('MemsblackCtrl', ['$scope', '$http','gridSharedService','$state','$filter', function($scope, $http,gridSharedService,$state, $filter) {
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        setTimeout(function () {
            ...
            if (searchText) {
                ...
                $http.get('member/members/blacklist').success(function (largeLoad) {
