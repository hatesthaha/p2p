...
app.controller('userroleMenuCtrl',['$scope', 'gridSharedService','$http', '$state', function($scope, gridSharedService, $http, $state){
    //编辑按钮组
    $scope.editGroup = {
        btnNew: true,
        btnEdit: false,
    ...
    $scope.opendellist = function(aciton){
        ...
        if(rows.length < 1){
            ...
            angular.forEach(rows, function(value, key) {
                rolename.push(value.name);
                rowid.push(value.id);
            });
            if(confirm('确定要删除此角色吗？删除后将无法恢复')){
                $http.post('user/rbac/role-delete', {role_name: rolename}).success(function (data) {
                    if(data){
                        alert("删除成功！");
                        $state.reload();
                    }else{
    ...
    $scope.openlistEditor = function(action){
        ...
        if(list){
            $state.go("app.user_rbac_role-view",{role_name:list.name});
        }else{
            $state.go("app.user_rbac_role-create");
        }
...
app.controller('userroleCtrl', ['$scope', '$http','gridSharedService','$state','$filter', function($scope, $http,gridSharedService,$state, $filter) {
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        setTimeout(function () {
            ...
            if (searchText) {
                ...
                $http.get('user/rbac/roles').success(function (largeLoad) {
