...
app.controller('activityMenuCtrl', ['$scope', 'gridSharedService', '$http', '$state', function ($scope, gridSharedService, $http, $state) {
    //编辑按钮组
    $scope.editGroup = {
        btnNew: true,
        btnEdit: false,
    ...
    $scope.opendellist = function (aciton) {
        ...
        if (rows.length < 1) {
            ...
            if (confirm('确定要删除此文章吗，删除后将无法恢复，请慎重')) {
                $http.post('invest/activity/delete', {id: rowid}).success(function (data) {
                    if (data) {
                        alert("删除成功！");
                        $state.reload();
                    } else {
    ...
    $scope.openlistEditor = function (action) {
        ...
        if (list) {
            $state.go("app.invest_activity_update", {id: list.id});
        } else {
            $state.go("app.invest_activity_create");
        }
...
app.controller('activieyCtrl', ['$scope', '$http', 'gridSharedService', '$state', '$filter', function ($scope, $http, gridSharedService, $state, $filter) {
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        setTimeout(function () {
            ...
            if (searchText) {
                ...
                $http.get('invest/activity/index').success(function (largeLoad) {
